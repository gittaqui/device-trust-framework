...
thresholds, and the experimental identity/behavior guard. All policies are evaluated
on identical generated rows so comparisons are paired.
...
Threshold, weight, missing-telemetry, and non-compensation studies are evaluated
separately so that changes in security outcomes can be distinguished from changes in
user friction and abstention behavior.
...
without increasing safe step-up relative to the 0.75 additive model. This apparent
friction advantage is not yet externally validated and is partly attributable to the
limited overlap between benign synthetic identity behavior and the guard region.
...
friction and false-allow metrics. External telemetry must be evaluated on temporally
held-out data with frozen proxy definitions and policy settings before stronger claims
are considered.
